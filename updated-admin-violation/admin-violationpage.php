...
// Handle adding new student violation
...
// Handle adding new violation type (for Configuration Tab)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['config_violation_type_submit'])) {
    $resolution = trim($_POST['configResolution'] ?? '');
    $violationTypeName = trim(strtoupper($_POST['configViolationType'] ?? '')); // Convert to uppercase
    $violationCategoryId = trim($_POST['configViolationCategory'] ?? ''); // Get selected category

    $response = ['success' => false, 'message' => 'An unexpected error occurred.'];

    if ($resolution && $violationTypeName && $violationCategoryId) {
        // First, check if the violation type already exists within this category
        $checkSql = "SELECT COUNT(*) FROM violation_type_tbl WHERE violation_type = ? AND violation_category_id = ?";
        $checkStmt = $conn->prepare($checkSql);
        if ($checkStmt) {
            $checkStmt->bind_param("si", $violationTypeName, $violationCategoryId);
            $checkStmt->execute();
            $checkResult = $checkStmt->get_result();
            $row = $checkResult->fetch_row();
            if ($row[0] > 0) {
                $response['message'] = 'Error: This violation type already exists in this category.';
            } else {
                // Prepare insert statement
                $insertSql = "INSERT INTO violation_type_tbl (violation_type, violation_category_id, resolution, date_added) VALUES (?, ?, ?, NOW())";
                $stmt = $conn->prepare($insertSql);

                if ($stmt) {
                    $stmt->bind_param("sis", $violationTypeName, $violationCategoryId, $resolution);
                    if ($stmt->execute()) {
                        $response['success'] = true;
                        $response['message'] = 'Violation type added successfully.';
                    } else {
                        $response['message'] = 'Error adding violation type: ' . htmlspecialchars($stmt->error);
                    }
                    $stmt->close();
                } else {
                    $response['message'] = 'Error preparing statement for violation type insert: ' . htmlspecialchars($conn->error);
                }
            }
            $checkStmt->close();
        } else {
            $response['message'] = 'Error preparing check statement for violation type: ' . htmlspecialchars($conn->error);
        }
    } else {
        $response['message'] = 'Please fill in all required fields for violation configuration.';
    }
...
<head>
    ...
    <link rel="stylesheet" href="../updated-admin-violation/admin_violation.css" />
...
<div class="container">
    ...
    <div class="tabs">
        <button class="tab active" data-tab="student-violations"><i class="fas fa-user-graduate"></i> Student</button>
        <button class="tab" data-tab="violation-config"><i class="fas fa-cogs"></i> Violation Configuration</button>
    </div>

    <div id="student-violations" class="tab-content active">
        <?php
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        if ($total_individual_violations > 0) {
                            foreach ($violations_by_type_count as $violation_id_key => $count) {
                                // Assuming 2 or more violations of the same type lead to sanction
                                if ($count >= 2) {
                                    $hasSanction = true;
                                    break;
                                }
    ...
    <div id="violation-config" class="tab-content">
        <h2>Violation Configuration</h2>

        <div class="config-controls">
            <button id="addConfigViolationBtn" class="add-violation-button"><i class="fas fa-plus"></i> Add New Violation Type</button>
        </div>

        <div class="violation-categories-container">
            <?php
            // Fetch violation categories from the database
            // Assuming you have a 'violation_category_tbl' with 'category_id' and 'category_name'
            $categorySql = "SELECT violation_category_id, category_name FROM violation_category_tbl ORDER BY category_name ASC";
            ...
            if ($categoryResult && $categoryResult->num_rows > 0) {
                ...
                // Fallback if no categories are found or if the table doesn't exist
                $categories = [
                    ['violation_category_id' => 1, 'category_name' => 'ID Violation'],
                    ['violation_category_id' => 2, 'category_name' => 'Registration Card/Certificate of Registration Violation'],
                    ['violation_category_id' => 3, 'category_name' => 'Prohibited Clothing Violation'],
                    ['violation_category_id' => 4, 'category_name' => 'Other Violation'],
                ];
            }

            foreach ($categories as $category) {
                $categoryId = htmlspecialchars($category['violation_category_id']);
                $categoryName = htmlspecialchars($category['category_name']);
            ?>
            <div class="violation-category-dropdown">
                <button class="category-dropdown-btn" data-category-id="<?php echo $categoryId; ?>">
                    <?php echo $categoryName; ?> <i class="fas fa-chevron-down"></i>
                </button>
                <div class="category-violation-list">
                    <table>
                        <thead>
                            <tr>
                                <th>Resolution</th>
                        ...
                        <tbody id="category-<?php echo $categoryId; ?>-violations-body">
                            <?php
                            // Fetch violation types for this category
                            $vtConfigSql = "SELECT violation_type_id, resolution, violation_type, date_added 
                                            FROM violation_type_tbl 
                                            WHERE violation_category_id = ? ORDER BY violation_type ASC";
                            $vtConfigStmt = $conn->prepare($vtConfigSql);
                            if ($vtConfigStmt) {
                                $vtConfigStmt->bind_param("i", $category['violation_category_id']);
                                $vtConfigStmt->execute();
                                $vtConfigResult = $vtConfigStmt->get_result();
                                if ($vtConfigResult->num_rows > 0) {
                                    while ($vtRow = $vtConfigResult->fetch_assoc()) {
                                        echo "<tr>";
                                        echo "<td>" . htmlspecialchars($vtRow['resolution']) . "</td>";
                                        echo "<td>" . htmlspecialchars($vtRow['violation_type']) . "</td>";
                                        echo "<td>" . htmlspecialchars(date('M d, Y', strtotime($vtRow['date_added']))) . "</td>";
                                        echo "<td>
                                                <button class='edit-config-violation-btn button' data-id='" . htmlspecialchars($vtRow['violation_type_id']) . "' data-resolution='" . htmlspecialchars($vtRow['resolution']) . "' data-type='" . htmlspecialchars($vtRow['violation_type']) . "' data-category='" . $categoryId . "'><i class='fas fa-edit'></i> Edit</button>
                                                <button class='delete-config-violation-btn button' data-id='" . htmlspecialchars($vtRow['violation_type_id']) . "'><i class='fas fa-trash-alt'></i> Delete</button>
                                              </td>";
                                        echo "</tr>";
                                    }
                                } else {
                                    echo "<tr><td colspan='4' class='no-records-cell'>No violation types found for this category.</td></tr>";
                                }
                                $vtConfigStmt->close();
                            } else {
                                error_log("Error preparing statement for violation types in config: " . $conn->error);
                                echo "<tr><td colspan='4' class='no-records-cell'>Error loading violation types.</td></tr>";
                            }
                            ?>
                        </tbody>
                    </table>
...
<div id="configViolationModal" class="modal" style="display:none;">
    <div class="modal-content">
        <div class="head-modal">
            <h3 id="configModalTitle">Add New Violation Type</h3>
            <span class="close-modal-button config-close-modal-button" style="float:right; cursor:pointer; font-size: 1.5em;">&times;</span>
        </div>
        <div id="configModalMessage" class="modal-message" style="display: none;"></div>
        
        <form id="configViolationForm" class="form-container">
            <input type="hidden" name="config_violation_type_submit" value="1">
            <input type="hidden" id="configViolationTypeId" name="configViolationTypeId" value="">
            <div class="row">
                <div class="column">
                    <label for="configViolationCategory">Violation Category:</label>
                    <select id="configViolationCategory" name="configViolationCategory" required>
                        <option value="">Select Category</option>
                        <?php
                        // Re-fetch categories for the config modal dropdown
                        $categorySqlModal = "SELECT violation_category_id, category_name FROM violation_category_tbl ORDER BY category_name ASC";
                        $categoryResultModal = $conn->query($categorySqlModal);
                        if ($categoryResultModal && $categoryResultModal->num_rows > 0) {
                            while ($catRowModal = $categoryResultModal->fetch_assoc()) {
                                echo '<option value="' . htmlspecialchars($catRowModal['violation_category_id']) . '">' 
                                        . htmlspecialchars($catRowModal['category_name']) . '</option>';
                            }
                        } else {
                            // Fallback options if no categories are found in DB
                            echo '<option value="1">ID Violation</option>';
                            echo '<option value="2">Registration Card/Certificate of Registration Violation</option>';
                            echo '<option value="3">Prohibited Clothing Violation</option>';
                            echo '<option value="4">Other Violation</option>';
                        }
                        ?>
                    </select>
                </div>
            ...
            <div class="row">
                <div class="column">
                    <label for="configResolution">Resolution (e.g., 1st Offense, Final Warning):</label>
                    <input type="text" id="configResolution" name="configResolution" required />
                </div>
                <div class="column">
                    <label for="configViolationType">Type of Violation:</label>
                    <input type="text" id="configViolationType" name="configViolationType" required style="text-transform: uppercase;" />
                </div>
            </div>
            <div class="button-row">
                <button type="submit" class="modal-button-add"><i class="fas fa-check"></i> <span id="configSubmitBtnText">Add</span></button>
                <button type="button" id="closeConfigModal" class="modal-button-cancel"><i class="fas fa-times"></i> Cancel</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    const currentPageUrl = '<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>';
</script>
<script src="../updated-admin-violation/admin_violation_page.js"></script>
